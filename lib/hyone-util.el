...
;; "[" and "]" must be first item because of emacs regex restriction
(setq hyone:bracket-pairs '(("[" . "]")    
                            ("(" . ")")
                            ("{" . "}")
                            ("|" . "|")))

(setq hyone:bracket-search-pattern-forward
      (concat "\\([" (apply 'concat (mapcar 'cdr hyone:bracket-pairs)) "]\\)"))

(setq hyone:bracket-search-pattern-backward
      (concat "\\([" (apply 'concat (mapcar 'car hyone:bracket-pairs)) "]\\)"))

(defun* hyone:in-blank-single-line-block-p (&key (has-space t))
  "whether or not be just in blank block"
  (let ((cur    (point))
    ...
    (goto-char cur)
    (if (save-excursion
          (re-search-forward (concat spaces hyone:bracket-search-pattern-forward) end t))
        (let ((rpos (match-beginning 0))
              (rstr (match-string-no-properties 1)))
          (if (save-excursion
                (re-search-backward (concat hyone:bracket-search-pattern-backward spaces) start t))
              (let ((lpos (match-end 0))
                    (lstr (match-string-no-properties 1)))
                (let ((counterpart (cdr (assoc lstr hyone:bracket-pairs))))
                  (and (= lpos cur)
                       (= rpos cur)
                       (equal counterpart rstr)))))))))
...
(defun* hyone:newline-dwim (&optional (newline-function 'newline)
  ...
  (interactive)
  ;; wrap newline-function to treat the case of skk henkan mode
  (let ((wraped-newline-function
         (lambda ()
    ...
    (if (hyone:in-blank-single-line-block-p)
        (progn
          (dotimes (i 2) (funcall wraped-newline-function))
          (funcall indent-function)
          (previous-line)
          (funcall indent-function))
      (funcall wraped-newline-function))))

(defun* hyone:insert-space-dwim (&optional (arg nil)
                                           (insert-function 'self-insert-command))
  "smart insert space in blank block ( i.g. {} )"
  ;; case of skk-henkan-mode on, <space> to start henkan,
  ;; otherwise, insert-function
  (let ((wraped-insert-function
         (lambda (arg)
           (if (and (featurep 'skk) skk-henkan-mode)
               (skk-insert)
    ...
    (if (hyone:in-blank-single-line-block-p :has-space nil)
        (progn
          (insert "  ")
          (backward-char))
      (funcall wraped-insert-function arg))))
